<form class="row g-3" action="{{ $menu->url }}/{{ $detail->id }}" method="POST">
    ...
    <div class="col-2" style="padding-left: 0; padding-bottom: 0;">
        ...
        <input type="color" class="form-control" id="back_color" name="back_color" value="{{ old('back_color', $detail->back_color) }}" style="padding: 0.1rem; margin: 0; height: 37px" disabled>
        @error('back_color')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-2" style="padding-left: 0; padding-bottom: 0;">
        ...
        <input type="color" class="form-control" id="fore_color" name="fore_color" value="{{ old('fore_color', $detail->fore_color) }}" style="padding: 0.1rem; margin: 0; height: 37px" disabled>
        @error('fore_color')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-12 mt-2">
        <input class="form-check-input" type="checkbox" value="1" id="main_status" name="main_status" @if (old('main_status', $detail->main_status) == 1) checked @endif disabled>
