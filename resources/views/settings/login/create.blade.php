                        ...
                        <form action="{{ $menu->url }}" method="POST">
                                    ...
                                    <select class="single-select form-control @error('user') is-invalid @enderror" id="user" name="user">
                                        ...
                                        @if ($users)
                                            @foreach ($users as $item)
                                                <option value="{{ $item->id }}" @if(old('user') == $item->id) selected @endif>{{ $item->nik }} | {{ $item->full_name }} ({{ $item->position->name }})</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('user')
                            ...
                            <div class="row mb-2">
                                <div class="col-6">
                                    ...
                                    <input type="password" class="form-control @error('password') is-invalid @enderror @if (session('field') == 'password') is-invalid @endif" id="password" name="password" value="{{ old('password') }}">
                                    @error('password')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-6">
                                    ...
                                    <input type="password" class="form-control @error('repassword') is-invalid @enderror @if (session('field') == 'password') is-invalid @endif" id="repassword" name="repassword" value="{{ old('repassword') }}">
                                    @error('repassword')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    @if (session('field') == 'password')
