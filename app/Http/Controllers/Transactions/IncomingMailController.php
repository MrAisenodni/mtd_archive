...
class IncomingMailController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->letter_file) {
            ...
            $fileName =  strtotime(now()).'_'.$request->letter_date.'_'.$request->letter_title.'_'.$request->sender_name.'.'.$extension;  // Concatenate both to get FileName
            $file->move(public_path(), $fileName);  
            $data['letter_file'] = $fileName; 
        }
    ...
    public function show($id)
    {
        $data = [
            'menu'          => $this->submenu->select('id', 'title', 'menu_id', 'url')->where('url', $this->path)->first(),
            'groups'        => $this->incoming_mail_group->select('id','name')->where('disabled', 0)->get(),
            'managers'      => $this->user->select('id','full_name')->where('disabled', 0)->get(),
            'detail'        => $this->incoming_mail->select('id', 'code', 'name', 'group_id', 'doc_ref', 'user_id')->where('id', $id)->where('disabled', 0)->first(),
        ];
